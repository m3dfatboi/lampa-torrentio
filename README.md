...
- Для сериалов опрашивает первые до 6 сезонов параллельно (`/stream/series/{imdb}:S:1.json`) — Torrentio для любого эпизода возвращает в том числе сезонные пакеты, поэтому покрытие нормальное.
...
```
[Torrentio] plugin source loaded v5-hide-date-peers
...
[Torrentio] Lampa.Template.get hooked v5-hide-date-peers
[Torrentio] Lampa.Parser.get hooked v5-hide-date-peers
```
