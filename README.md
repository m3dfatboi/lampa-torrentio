...
- Для сериалов берёт IMDb ID через TMDB TV endpoint, тянет bulk `/stream/series/{imdb}.json` и дополнительно опрашивает известные episode endpoints (`/stream/series/{imdb}:S:E.json`), чтобы Torrentio вернул сезонные паки и отдельные серии даже когда bulk пустой. Запросы ограничены первыми 80 endpoint'ами, чтобы не перегружать клиент.
...
```
[Torrentio] plugin source loaded v14-series-episodes
...
[Torrentio] Lampa.Template.get hooked v14-series-episodes
[Torrentio] Lampa.Parser.get hooked v14-series-episodes
```
